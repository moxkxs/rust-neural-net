...
impl Network {
    pub fn build(nn_arch: &Array1<u64>) -> Result<Network, &'static str> {
        ...
        for l in nn_arch.iter().skip(1) {
            biases.push(Array::random((*l as usize, 1), rand_distr::Normal::new(0.0, 1.0).expect("biases not initiating")));
        }

        let mut weights: Vec<Array2<f64>> = Vec::new();
        for (x, y) in nn_arch.iter().zip(nn_arch.iter().skip(1)) {
            weights.push(Array::random((*y as usize, *x as usize), rand_distr::Normal::new(0.0, 1.0).expect("biases not initiating")));
        }
